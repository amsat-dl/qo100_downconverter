...
void lcd_init(i2c_dev i2c, u8 addr)
{
 ...
 ssd1306_commandbyte(0x40 | 0x0);


 lcd_clear_screen(0);
 print_string("AMSAT-DL  QO-100",0,0,0);
 print_string("WELCOME to:",0,2,0);
 print_string("GPS/OCXO clock",0,4,0);
 print_string("LNB Controller",0,5,0);
 print_string("Downconverter",0,6,0);
}
...
void lcd_clear_screen(u8 ystart)
{
 ...
 memset(buf,0,128);
    ...
    for (u8 row=ystart; row<(64 / 8); row++)
 {
...
void show_display()
{
 ...
 static u8 old_lnbqrg = 0;
 static u8 old_GPS_sat = 0xff;
 static char old_GPS_lat[15] = {"             "};
 static char old_GPS_lon[15] = {"             "};
 ...
 if(booting == 1 && (!adf4351_lockstat || !si5328locked || !LNB_laststat))
 {
  ...
  if(LNB_laststat) print_string("OK",11*8,4,0); else print_string(itoa(LNB_error),11*8,4,0);
 ...
 u8 lnbqrg = LO[currentLO-24];

 if(old_GPS_lock != GPS_lock)
 {
  if(!GPS_lock)
  {
 ...
 if(old_outqrg != outqrg)
 {
  ...
  print_string(".55", 8*8, 3, 0);
  print_string("MHz",12*8,3,0);

  old_outqrg = outqrg;
 }

 if(old_lnbqrg != lnbqrg)
 {
  ...
  {
   print_string(itoa(lnbqrg), 6*8, 4, 0);
 ...
 if(GPS_lock)
 {
  if(old_GPS_sat != GPS_sat)
  {
   ...
   p = itoa(GPS_sat);
   xpos = 8-strlen(p);
   print_string("       ",4*8,5,0);
   print_string(itoa(GPS_sat),xpos*8,5,0);

   old_GPS_sat = GPS_sat;
  }

  if(memcmp(old_GPS_lat,GPS_lat,15))
  {
   print_string("LAT:",0,6,0);
   print_string(GPS_lat,8*6,6,0);

   memcpy(old_GPS_lat,GPS_lat,15);
  }

  if(memcmp(old_GPS_lon,GPS_lon,15))
  {
   print_string("LON:",0,7,0);
   print_string(GPS_lon,8*5,7,0);

   memcpy(old_GPS_lon,GPS_lon,15);
  }
 }
}
