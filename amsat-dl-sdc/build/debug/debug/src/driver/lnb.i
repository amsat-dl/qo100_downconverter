...
static int lnb_do_configure(i2c_dev i2c, u8 addr)
{
    ...
    res = smbus8_write_byte(i2c, addr, 0x02, 0x84);
    if(res == 0)
 {
  res = smbus8_read_byte(i2c, addr, 0x05, &value);
  if(res == 0)
  {
   value |= (1<<6) | (1<<3);
   res = smbus8_write_byte(i2c, addr, 0x05, value);
   if(res == 0)
   {
    res = smbus8_read_byte(i2c, addr, 0x04, &value);
    if(res == 0)
    {
     value |= (1<<2) | (1<<6);
     res = smbus8_write_byte(i2c, addr, 0x04, value);
    }
   }
  }
 }
...
u8 check_LNB_status(i2c_dev i2c, u8 addr)
{
 ...
 stat = ((*((PIN_LNB_FAULT)->pPORT) & (PIN_LNB_FAULT)->mask) != 0);
