...
# 6 "src/driver/si5328.c" 2
# 39 "src/driver/si5328.c"
u8 si5328locked = 0;
...
# 76 "src/driver/si5328.c"
static const reg_set reg_set_univ[] = {
# 88 "src/driver/si5328.c"
    { 0, 0x14 | 0x40},
 { 1, 0xe0 | 0x04 | 0x00},
 { 2, 0x02 | 0x40},
 { 3, 0x05 | 0x40},
 ...
 {40, (10 -4)<<5}, {41, (240-1)>>8}, {41 +1, (240-1)&0xFF},
 ...
 {25, (5 -4)<<5},
    ...
    {31, 0}, {32, 40-1>>8}, {32 +1, 40-1&0xFF},
    {34, 0}, {35, 24-1>>8}, {35 +1, 24-1&0xFF},
...
static reg_set reg_set_LNB_LO_24[] =
{
 {25, (5 -4)<<5},
 {33, 40-1},
 {36, 24-1},
...
static reg_set reg_set_LNB_LO_25[] =
{
 {25, (4 -4)<<5},
 {33, 48-1},
 {36, 30-1},
...
static reg_set reg_set_LNB_LO_26[] =
{
 {25, (5 -4)<<5},
 {33, 40-1},
 {36, 26-1},
...
static reg_set reg_set_LNB_LO_27[] =
{
 {25, (10 -4)<<5},
 {33, 22-1},
 {36, 14-1},
...
int si5328_init(i2c_dev i2c, u8 addr)
{
 ...
 {
        printf("Si5328 found.\n");
  smbus8_write_byte(i2c, addr, 136, 0x80);
  _delaywdt((unsigned long)((100)*(8000000/4000.0)));

  printf("Si5328 init\n");
        write_regset(i2c, addr, reg_set_univ, (sizeof(reg_set_univ)/sizeof(reg_set_univ[0])));
...
void si5328_set_freq(i2c_dev i2c, u8 addr, u8 freq_MHz)
{
 ...
 if(freq_MHz == 24) p= reg_set_LNB_LO_24;
 if(freq_MHz == 25) p= reg_set_LNB_LO_25;
 if(freq_MHz == 26) p= reg_set_LNB_LO_26;
 if(freq_MHz == 27) p= reg_set_LNB_LO_27;

 if(p != ((void*)0)) {
  write_regset(i2c, addr, p, (sizeof(reg_set_LNB_LO_24)/sizeof(reg_set_LNB_LO_24[0])));
  printf("Si5328 LNB output frequency set to %d MHz\n",freq_MHz);
  si5328_perform_ical(i2c, addr);

 }
}
# 248 "src/driver/si5328.c"
static void si5328_perform_ical(i2c_dev i2c, u8 addr)
{
 printf("Si5328 ical\n");
 if (smbus8_write_byte(i2c, addr, 136, 0x40) == 0)
 {
  printf("Si5328 ICAL in process\n");
 }
 else
 {
  printf("Si5328 ICAL write failed.\n");
 }
}
...
u8 check_si5328_status(i2c_dev i2c, u8 addr)
{
 ...
 u8 lock = ((*((PIN_SI5328_LOL)->pPORT) & (PIN_SI5328_LOL)->mask) != 0);
 if(lock != lastlock)
 {
  lastlock = lock;
  if(lock == 1)
...
# 304 "src/driver/si5328.c"
 static u8 lastinput = 0xff;
 res = read_reg(i2c, addr, 128);
 if(res != lastinput)
 {
  if(!(res & 1) && !(res & 2)) printf("Si5328: Input = disconnected !\n");
  if(res & 1) printf("Si5328: Input = CKIN1 (GPS)\n");
  if(res & 2) printf("Si5328: Input = CKIN2 (TCXO/OCXO)\n");
  lastinput = res;
 }
# 325 "src/driver/si5328.c"
 return ret;
}
...
static void write_regset(i2c_dev i2c, u8 addr, const reg_set *set, u16 len)
{
  ...
  if(errors > 0)
 {
  printf("Si5328 write_regset %i errors.\n", errors);
 }
}
