...
# 5 "src/gpio.c" 2
# 1 "src/driver/si5328.h" 1



# 1 "src/driver/base/i2c.h" 1
# 17 "src/driver/base/i2c.h"
...
struct i2c_dev__ { int unused; }; typedef struct i2c_dev__ *i2c_dev;
...
void init_gpios(void)
{
    ...
    *((PIN_ADF_LOCK)->pTRIS) |= (PIN_ADF_LOCK)->mask;

    *((PIN_ADF_CE)->pPORT) &= ~(PIN_ADF_CE)->mask;
    *((PIN_ADF_CE)->pTRIS) &= ~(PIN_ADF_CE)->mask;

    *((PIN_ADF_LE)->pPORT) &= ~(PIN_ADF_LE)->mask;
    *((PIN_ADF_LE)->pTRIS) &= ~(PIN_ADF_LE)->mask;

    *((PIN_SIO_DATA)->pPORT) &= ~(PIN_SIO_DATA)->mask;
    *((PIN_SIO_DATA)->pTRIS) &= ~(PIN_SIO_DATA)->mask;

    *((PIN_SIO_CLK)->pPORT) &= ~(PIN_SIO_CLK)->mask;
    *((PIN_SIO_CLK)->pTRIS) &= ~(PIN_SIO_CLK)->mask;



 *((PIN_SI5328_LOL)->pTRIS) |= (PIN_SI5328_LOL)->mask;

    *((PIN_SI5328_CS_CA)->pPORT) |= (PIN_SI5328_CS_CA)->mask;
    *((PIN_SI5328_CS_CA)->pTRIS) |= (PIN_SI5328_CS_CA)->mask;


    *((PIN_LED_SI_LOCK)->pPORT) |= (PIN_LED_SI_LOCK)->mask;
    *((PIN_LED_SI_LOCK)->pTRIS) &= ~(PIN_LED_SI_LOCK)->mask;

    *((PIN_LED_CPU_OK)->pPORT) |= (PIN_LED_CPU_OK)->mask;
    *((PIN_LED_CPU_OK)->pTRIS) &= ~(PIN_LED_CPU_OK)->mask;

    *((PIN_LED_ADF_LOCK)->pPORT) |= (PIN_LED_ADF_LOCK)->mask;
    *((PIN_LED_ADF_LOCK)->pTRIS) &= ~(PIN_LED_ADF_LOCK)->mask;

    *((PIN_LED_LNB_GOOD)->pPORT) |= (PIN_LED_LNB_GOOD)->mask;
    *((PIN_LED_LNB_GOOD)->pTRIS) &= ~(PIN_LED_LNB_GOOD)->mask;


    *((PIN_I2C_SCL)->pTRIS) |= (PIN_I2C_SCL)->mask;
    *((PIN_I2C_SDA)->pTRIS) |= (PIN_I2C_SDA)->mask;


 check_RXTX();


    *((PIN_SJ3)->pTRIS) |= (PIN_SJ3)->mask;
    *((PIN_SJ3)->pPORT) |= (PIN_SJ3)->mask;
    *((PIN_SJ4)->pTRIS) |= (PIN_SJ4)->mask;
    *((PIN_SJ4)->pPORT) |= (PIN_SJ4)->mask;
    *((PIN_SJ5)->pTRIS) |= (PIN_SJ5)->mask;
    *((PIN_SJ5)->pPORT) |= (PIN_SJ5)->mask;
    *((PIN_SJ6)->pTRIS) |= (PIN_SJ6)->mask;
    *((PIN_SJ6)->pPORT) |= (PIN_SJ6)->mask;


    *((PIN_LNB_QRG2)->pTRIS) |= (PIN_LNB_QRG2)->mask;
    *((PIN_LNB_QRG2)->pPORT) |= (PIN_LNB_QRG2)->mask;
    *((PIN_LNB_QRG1)->pTRIS) |= (PIN_LNB_QRG1)->mask;
    *((PIN_LNB_QRG1)->pPORT) |= (PIN_LNB_QRG1)->mask;


    *((PIN_LNB_FAULT)->pTRIS) |= (PIN_LNB_FAULT)->mask;
}


void check_RXTX()
{
 *((PIN_USART_TX)->pTRIS) &= ~(PIN_USART_TX)->mask;
    *((PIN_USART_RX)->pTRIS) |= (PIN_USART_RX)->mask;


 for(int i=0; i<5; i++)
 {
  *((PIN_USART_TX)->pPORT) &= ~(PIN_USART_TX)->mask;
  _delaywdt((unsigned long)((10)*(8000000/4000.0)));
  if(((*((PIN_USART_RX)->pPORT) & (PIN_USART_RX)->mask) != 0))
  {
   ext10MHzReference = 0;
   return;
  }
  ...
  *((PIN_USART_TX)->pPORT) |= (PIN_USART_TX)->mask;
  _delaywdt((unsigned long)((10)*(8000000/4000.0)));
  if(((*((PIN_USART_RX)->pPORT) & (PIN_USART_RX)->mask) != 0) == 0)
  {
   ext10MHzReference = 0;
   return;
  }
