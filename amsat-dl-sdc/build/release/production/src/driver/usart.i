...
# 2 "<built-in>" 2
# 1 "src/driver/usart.c" 2
# 15 "src/driver/usart.c"
# 1 "src/config.h" 1
# 11 "src/config.h"
...
{
    irq_set_handler(IRQ_HANDLER_ID_USART, usart_rx_irq_handler, ((void*)0));
    TXSTAbits.TX9 = 0;
    TXSTAbits.SENDB = 0;
    TXSTAbits.BRGH = 1;
    TXSTAbits.TXEN = 1;
    ...
    u16 spbrg_value = ((8000000/4) / baud) - 1;

    SPBRGH = (spbrg_value>>8)&0xFF;
    SPBRG = (spbrg_value>>0)&0xFF;
...
char usart_putc(char c)
{
 ...
 __asm(" clrwdt");
    while(TXSTAbits.TRMT == 0) __asm(" clrwdt");
